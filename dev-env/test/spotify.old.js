...
Spotify.prototype._makeLandingPageRequest = function () {
	...
	debug("GET %j", url);
	this.agent.get(url).set({ "User-Agent": this.userAgent }).end(this._onsecret.bind(this));
};
...
Spotify.prototype._onsecret = function (err, res) {
	...
	for (var i = 0; i < scripts.length; i++) {
		...
		if (~code.indexOf("Spotify.Web.Login")) {
			vm.runInNewContext(code, {
				document: null,
				Spotify: { Web: { Login: login, App: { initialize: function () {} } } },
			});
		}
	}
	...
	debug("POST %j", url);
	this.agent.post(url).set({ "User-Agent": this.userAgent }).type("form").send(creds).end(this._onauth.bind(this));
};
...
Spotify.prototype._resolveAP = function () {
	...
	debug("GET %j", url);
	this.agent.get(url).set({ "User-Agent": this.userAgent }).query(query).end(this._openWebsocket.bind(this));
};
...
Spotify.prototype.sendProtobufRequest = function (req, fn) {
	...
	var loadSchema = function (schema, dontRecurse) {
		if ("string" === typeof schema) {
			...
			if (!schema) throw new Error("Could not load schema: " + schemaName.join("#"));
...
Spotify.prototype.get = Spotify.prototype.metadata = function (uris, fn) {
...
Spotify.prototype.playlist = function (uri, from, length, fn) {
	...
	var user = parts[2];
	var id = parts[4];
	var hm = "hm://playlist/user/" + user + "/playlist/" + id + "?from=" + from + "&length=" + length;
...
Spotify.prototype.similar = function (uri, fn) {
	...
	if (!type || !id || "track" != type) throw new Error("uri must be a track uri");
